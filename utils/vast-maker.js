...
/**
 *
 * @param {Object} opt - Contains properties used to build the tracking end-point.
 * @returns {Object} Object containing "xml" and "duration" properties.
 *
 * @example
 * opt = { sessionId: "abc-123",
 * desiredDuration: "60",
...
function VastBuilder(opt) {
  ...
  const [selectedAds, adsDuration] = GetAdsAndDuration(opt.desiredDuration);
  ...
  for (let i = 0; i < selectedAds.length; i++) {
    vast4
      ...
      .addTracking(
        `http://${opt.adserverHostname}/api/v1/sessions/${opt.sessionId}/tracking?adId=${selectedAds[i].id}&progress=0`,
        { event: "start" }
      )
      .addTracking(
        `http://${opt.adserverHostname}/api/v1/sessions/${opt.sessionId}/tracking?adId=${selectedAds[i].id}&progress=25`,
        { event: "firstQuartile" }
      )
      .addTracking(
        `http://${opt.adserverHostname}/api/v1/sessions/${opt.sessionId}/tracking?adId=${selectedAds[i].id}&progress=50`,
        { event: "midpoint" }
      )
      .addTracking(
        `http://${opt.adserverHostname}/api/v1/sessions/${opt.sessionId}/tracking?adId=${selectedAds[i].id}&progress=75`,
        { event: "thirdQuartile" }
      )
      .addTracking(
        `http://${opt.adserverHostname}/api/v1/sessions/${opt.sessionId}/tracking?adId=${selectedAds[i].id}&progress=100`,
        { event: "complete" }
      )
      .and()
