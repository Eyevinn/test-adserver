...
function VastBuilder(params) {
  ...
  AttachStandAloneAds(
    vast4,
    selectedAds.standAloneAds,
    params,
    selectedAds.podAds.length
  );
...
function AttachPodAds(vast4, podAds, params) {
  for (let i = 0; i < podAds.length; i++) {
    vast4
      ...
      .attachTrackingEvents()
      .addTracking(
        `http://${params.adserverHostname}/api/v1/sessions/${
          params.sessionId
        }/tracking?adId=${podAds[i].id}_${i + 1}&progress=0`,
        { event: 'start' }
      )
      .addTracking(
        `http://${params.adserverHostname}/api/v1/sessions/${
          params.sessionId
        }/tracking?adId=${podAds[i].id}_${i + 1}&progress=25`,
        { event: 'firstQuartile' }
      )
      .addTracking(
        `http://${params.adserverHostname}/api/v1/sessions/${
          params.sessionId
        }/tracking?adId=${podAds[i].id}_${i + 1}&progress=50`,
        { event: 'midpoint' }
      )
      .addTracking(
        `http://${params.adserverHostname}/api/v1/sessions/${
          params.sessionId
        }/tracking?adId=${podAds[i].id}_${i + 1}&progress=75`,
        { event: 'thirdQuartile' }
      )
      .addTracking(
        `http://${params.adserverHostname}/api/v1/sessions/${
...
function GetAdsAndDuration(adList, targetDuration, podSize, podMin, podMax) {
  ...
  const actualPodDuration = chosenPodAds
    .map((ad) => timestampToSeconds(ad.duration))
    .reduce((a, b) => a + b, 0);
  ...
  const actualDuration =
    standAloneTargetDuration - remainingDuration + actualPodDuration;
  return [
    { standAloneAds: chosenStandAloneAds, podAds: chosenPodAds },
  ...
  switch (_method) {
    case PopulationMethods.GO_BY_SIZE_AND_MAX:
      ...
      PopulatePod(
        _size - 1,
        _min,
        _max - allDurations[chosenIdx],
        _ads,
        _chosenAds,
        PopulationMethods.GO_BY_SIZE_AND_MAX,
        _targetDur
      );
      break;
    case PopulationMethods.GO_BY_MAX:
      ...
      while (
        ...
        remainingDuration >= shortestAdDuration
      ) {
        // Adding to chosenStandAloneAds list based on durations; Add the longest Ad possible.
